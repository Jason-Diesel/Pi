...
bool loadCShader(const std::string& name, uint32_t& computeShader)
{
	std::string shaderData;
	if (!readShader(name, shaderData)) {
		return false;
	...
	int res;
	glGetShaderiv(computeShader, GL_COMPILE_STATUS, &res);
	if (res == GL_FALSE) {
		readShaderError(computeShader, GL_FRAGMENT_SHADER);
		return false;
	}

	uint32_t p = glCreateProgram();
	glAttachShader(p, computeShader);
	glLinkProgram(p);

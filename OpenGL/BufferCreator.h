...
template <typename T>
uint32_t CreateNumberBuffer(std::vector<T> numbers, GLenum type = GL_BYTE) {
//uint32_t CreateNumberBuffer(std::vector<T> numbers, GLenum type = GL_INT) {
    uint32_t buffer = 0;
    GLTest(glBindBuffer(GL_ARRAY_BUFFER, 0));
    GLTest(glGenBuffers(1, &buffer));
    GLTest(glBindBuffer(GL_ARRAY_BUFFER, buffer));
    GLTest(glBufferData(GL_ARRAY_BUFFER, sizeof(T) * numbers.size(), numbers.data(), GL_STATIC_DRAW));

    GLTest(glEnableVertexAttribArray(0));
    GLTest(glVertexAttribPointer(0, 1, type, GL_FALSE, sizeof(T), (void*)0));

    GLTest(glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, buffer));
    GLTest(glBindBuffer(GL_ARRAY_BUFFER, 0));
    return buffer;
}

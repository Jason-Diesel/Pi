...
void ImGuiManager::setPiUpdate(PiUpdate* piUpdate)
{
	this->piUpdate = piUpdate;
}

void ImGuiManager::renderPiUpdate()
{
	int temp = 0;
	ImGui::Begin("OpenGL Texture Text");

	temp = piUpdate->currentWidth;
	ImGui::SliderInt("Width", &temp, 0, piUpdate->textureWidth);
	piUpdate->currentWidth = temp;

	temp = piUpdate->groupsOf;
	ImGui::SliderInt("Group", &temp, 1, 5);
	piUpdate->groupsOf = temp;

	bool color = piUpdate->color == 1 ? true : false;
	ImGui::Checkbox("Color", &color);
	piUpdate->color = color ? 1 : 0;

	temp = piUpdate->offset;
	ImGui::InputInt("Offset", &temp, 1, 100);
	piUpdate->offset = temp;

	ImGui::SliderFloat("Zoom", &piUpdate->zoom, 1, 20);
